...
if($logged){
	...
	switch($act){
		...
		case "fall":
			...
			$sqlg_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.friend_id, tb2.user_id, user_search_pref, user_photo, user_country_city_name, user_status FROM `".PREFIX."_friends` tb1, `".PREFIX."_users` tb2 WHERE tb1.friend_id = '{$f_user_id}' AND tb1.user_id = tb2.user_id AND tb1.subscriptions = 1 ORDER by `friends_date` DESC LIMIT 0, 40", 1);
			...
			if($sqlg_){
				...
				foreach($sqlg_ as $rowg){
					...
					echo "<div class=\"onefriend\"><a href=\"/u{$rowg['user_id']}\" onclick=\"Page.Go(this.href); return false\"><div><img src=\"{$ava}\"></div>{$friend_info_online[0]}<br><span>{$friend_info_online[1]}</span></a></div>";
