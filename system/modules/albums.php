...
HTML;
		...
		case "all_comments":
			...
			if($privacy AND !$CheckBlackList){
				...
				if($uid AND !$aid)
					$sql_ = $db->super_query("SELECT SQL_CALC_FOUND_ROWS tb1.user_id, text, date, id, hash, album_id, pid, owner_id, photo_name, tb2.user_search_pref, user_photo, user_last_visit FROM `".PREFIX."_photos_comments` tb1, `".PREFIX."_users` tb2 {$sql_tb3} WHERE tb1.owner_id = '{$uid}' AND tb1.user_id = tb2.user_id {$privacy_sql} ORDER by `date` DESC LIMIT {$limit_page}, {$gcount}", 1);
				else
				...
				if(!$CheckBlackList){
					...
					if($sql_){
						...
						if($row_owner['user_albums_num']){
							if($user_info['user_id'] == $uid){
								$num = 'У Вас <span id="albums_num">'.$row_owner['user_albums_num'].'</span> '.gram_record($row_owner['user_albums_num'], 'albums');
							} else {
								$num = 'У '.gramatikName($author_info[0]).' '.$row_owner['user_albums_num'].' '.gram_record($row_owner['user_albums_num'], 'albums');
							}
