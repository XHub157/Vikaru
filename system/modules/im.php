...
$tpl->set_block("'\\[none\\](.*?)\\[/none\\]'si","");
			...
			$tpl->compile('content');
			foreach($sql_ as $row){
				$sql_f = $db->super_query("SELECT tb1.date, text,from_user_id,folder,pm_read FROM `".PREFIX."_messages` tb1, `".PREFIX."_users` tb2 WHERE tb1.from_user_id = '".$user_id."' AND tb1.for_user_id = '".$row['im_user_id']."' ORDER BY `date` DESC LIMIT 1");
				$tpl->load_template('im/im.tpl');
				$tpl->set('{name}', $row['user_search_pref']);
				$tpl->set('{date}', megaDateNoTpl($sql_f['date']));
				$tpl->set('{text}', $sql_f['text']);
  ...
  <table cellpadding="0" cellspacing="0" class="dialogs_row_t">
    <tbody><tr>
      <td class="dialogs_photo">
        <a href="/u{uid}" target="_blank"><img src="{ava}" width="50" height="50"></a>
      </td>
      <td class="dialogs_info">
        <div class="dialogs_user wrapped"><a href="/u{uid}" target="_blank">{name}</a></div>
