...
if($logged){
    ...
    switch($act){
		...
		if($db_ads_all){
			...
			foreach($db_ads_all as $row_ads){
				...
				//Информируем выделевшы обявление желтым, когда просмотров осталось 5..
				if($row_ads['views'] == '5'){

					$tpl->set('{style}', 'style="background: #F3EFE5;border: 1px solid #D8C5B8;"');

				}
				//Удаляем обявление если просмотры равны к нулю.
				...
				msgbox('', 'На данный момент нету объявлений ', 'info_2');
		break;
        ...
        case "ads_view":
            ...
            if($upload_ads){
				...
				<a href="/ads&act=ads" onClick="ads.ajax(); return false" class="size10 infowalltext_f clear">Все объявления</a>
    ...
    ';
		...
		//Удаление обявления юзера + возврат средств (голосов)
		case "delete_ads":
