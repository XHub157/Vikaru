<?php
/*
	Appointment: Редактирование игр
	File: editapp.php
	Autor: Sofwar
	Email: ya.sofwar@yandex.ru
...
if($logged){
	...
	switch($act){
		...
		case "do_create":
			...
			$db->query("INSERT INTO `".PREFIX."_apps` (`id`,`title`,`desk`,`user_id`) VALUES ('','".$title."','".$desk."','".$user_id."') "); 
			$dbid = $db->insert_id();
			echo $dbid;
		break;
		...
		case "info":
			...
			$id = intval($_GET['id']);
			$row = $db->super_query("SELECT id, title, desk, img,user_id,real_admin FROM `".PREFIX."_apps` WHERE id='{$id}'");
			$array_admin = explode('|', $row['real_admin']);
		...
		case "options":
			...
			$row = $db->super_query("SELECT id, title, desk, img,width,status,height,url,user_id,real_admin,type FROM `".PREFIX."_apps` WHERE id='{$id}'");
		...
		case "save_info":
			...
			if($app_hash == md5($id.'_'.$key)){
				if($title == '') die("name");
				elseif($desk == '') die("desk");

				$db->query("SET NAMES 'utf8'"); // Я почему то не смог нормально записать)

				$db->query("UPDATE `".PREFIX."_apps` SET `title`='{$title}', `desk`='{$desk}' WHERE `id`='{$id}'");
				echo "ok";
			}else{
				$tpl->load_template('apps/editapp/error.tpl');
				$tpl->compile('content');
		...
		case "save_options":
			...
			if($app_hash == md5($id.'_'.$key)){
				$db->query("UPDATE `".PREFIX."_apps` SET `status`='{$status}', `url`='{$url}', `width`='{$width}', `height`='{$height}', `type`='{$type}' WHERE `id`='{$id}'");
				echo "ok";
			}else{
				$tpl->load_template('apps/editapp/error.tpl');
				$tpl->compile('content');
		...
		case "payments":
			$metatags['title'] = 'Редактирование приложения';
			$id = intval($_GET['id']);
			$row = $db->super_query("SELECT `id`,`user_id`,`balance`,`admin` FROM `".PREFIX."_apps` WHERE `id`='{$id}'");

			$array_admin = explode('|', $row['admin']);

			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){
				$row1 = $db->super_query("SELECT `id`, `votes`, `from`, `whom`,`date`,`game_id` FROM `".PREFIX."_apps_transactions` WHERE `id`='{$id}' ORDER BY id DESC LIMIT 20",1);
		...
		case "admins":
			...
			$row = $db->super_query("SELECT `id`,`user_id`,`admin` FROM `".PREFIX."_apps` WHERE `id`='{$id}'");
			$array_admin = explode('|', $row['admin']);

			if(in_array($user_id, $array_admin) or $user_id == $row['user_id'] ){

				$array_admins = explode('|', $row['admin']);
				...
				foreach($array_admins as $user){
					if($user){
						...
						if($infoUser['user_photo'])
							...
							$tpl->set('{img}', '{theme}/images/no_ava_50.png');
		...
		case 'save_admin':
			...
			if($app_hash == md5($id.'_'.$key)){
				...
				$myRow = $db->super_query("SELECT admin,user_id FROM `".PREFIX."_apps` WHERE id = '{$id}'");
				$array_admin = explode('|', $myRow['admin']);
				if(!in_array($addr, $array_admin) AND $user_id != $addr AND $myRow['user_id'] != $addr){
					$db->query("UPDATE `".PREFIX."_apps` SET real_admin = real_admin+1, admins = '{$myRow['admin']}|{$addr}|' WHERE id = '{$id}'");
					echo 'ok';
				} else echo "not";
    	...
    	case 'del_admin':
			...
			if($app_hash == md5($id.'_'.$key)){
				...
				if(in_array($addr, $array_admin) AND $user_id != $addr){
					$myRow['admin'] = str_replace("|{$addr}|", "", $myRow['admin']);
					$db->query("UPDATE `".PREFIX."_apps` SET real_admin = real_admin-1, admins = '{$myRow['admin']}' WHERE id = '{$id}'");
					echo 'ok';
				} else echo "not";
