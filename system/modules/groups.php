	...
	switch($act){
		...
		case "send":
			...
			if(isset($title) AND !empty($title)){
				...
				$db->query("INSERT INTO `".PREFIX."_communities` SET title = '{$title}', type = 1, traf = 1, ulist = '|{$user_id}|', date = NOW(), admin = 'u{$user_id}|', real_admin = '{$user_id}', time={$server_time}, comments = 1, privacy = 'p_contact|1||p_audio|1||'");
				$cid = $db->insert_id();
				...
				if(in_array(strtolower($type), $allowed_files)){
					if($image_size < 5000000){
						...
						if(move_uploaded_file($image_tmp, $upload_dir.$image_rename.$res_type)){
							...
							//Добавляем на стену
							$wall_text = "<div class=\"profile_update_photo\"><a href=\"\" onClick=\"epage.vPhoto(\'{$id}\', \'{$image_rename}{$res_type}\'); return false\"><img src=\"/uploads/groups/{$id}/{$image_rename}{$res_type}\" style=\"margin-top:3px\"></a></div>";
		...
		case "wall_like_users_five":
			...
			if($sql_){
				foreach($sql_ as $row){
					...
					echo '<a href="/u'.$row['user_id'].'" id="Xlike_user'.$row['user_id'].'_'.$rec_id.'" onClick="Page.Go(this.href); return false"><img src="'.$ava.'" width="32" title="'.$row['user_search_pref'].'"/></a>';
				}
			}
			die();
		break;
		...
		case "all_groups_user":
			...
			if($sql_){
				...
				foreach($sql_ as $row){
					...
					$tpl->set('{status}', stripslashes($row['status_text']));
