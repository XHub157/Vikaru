...
if($logged){
	...
	if($type == 1){ //Если критерий поиск "по людям"
		$sql_query = "SELECT SQL_CALC_FOUND_ROWS user_id, user_search_pref, user_real, user_photo, user_birthday, user_country_city_name, user_last_visit, user_vuz FROM `".PREFIX."_users` WHERE user_search_pref LIKE '%{$query}%' {$sql_sort} ORDER by `user_reg_date` DESC LIMIT {$limit_page}, {$gcount}";
		...
		$sql_query = "SELECT SQL_CALC_FOUND_ROWS id, com_real, title, photo, traf, adres, gtype FROM `".PREFIX."_communities` WHERE title LIKE '%{$query}%' ORDER by `traf` DESC, `photo` DESC LIMIT {$limit_page}, {$gcount}";
		...
		$sql_query = "SELECT SQL_CALC_FOUND_ROWS `".PREFIX."_audio`.aid, url, name, artist, auser_id, ".PREFIX."_users.user_search_pref FROM ".PREFIX."_audio LEFT JOIN ".PREFIX."_users ON ".PREFIX."_audio.auser_id = ".PREFIX."_users.user_id WHERE name LIKE '%{$query}%' AND stype = 0 ORDER by `adate` DESC LIMIT {$limit_page}, {$gcount}"; 
		$sql_count = "SELECT COUNT(*) AS cnt FROM `".PREFIX."_audio` WHERE name LIKE '%{$query}%' AND stype = 0"; 
	...
	//################## Загружаем Страны ##################//
	$sql_country = $db->super_query("SELECT SQL_CALC_FOUND_ROWS * FROM `".PREFIX."_country` ORDER by `id` ASC", true, "country", true);
	...
	//################## Загружаем Города ##################//
	if($type == 1){
		$sql_city = $db->super_query("SELECT SQL_CALC_FOUND_ROWS id, name FROM `".PREFIX."_city` WHERE id_country = '{$country}' ORDER by `id` ASC", true, "country_city_".$country, true);
