...
if($_GET['act'] == 'edit'){
	...
	$row = $db->super_query("SELECT title, descr, comments, photo FROM `".PREFIX."_clubs` WHERE id = '".$id."'");
	if($row){
		if(isset($_POST['save'])){
			...
			if(isset($title) AND !empty($title) AND isset($descr) AND !empty($descr)){
				...
				if($_POST['del_photo']){
					@unlink(ROOT_DIR.'/uploads/clubs/'.$id.'/'.$row['photo']);
					$sql_line_del = ", photo = ''";
				}
				
				$db->query("UPDATE `".PREFIX."_clubs` SET title = '".$title."', descr = '".$descr."', comments = '".$comments."' ".$sql_line_del." WHERE id = '".$id."'");
				msgbox('Информация', 'Сообщество успешно отредактировано', '?mod=clubs');
			} else
				msgbox('Ошибка', 'Заполните все поля', '?mod=clubs&act=edit&id='.$id);
		} else {
			...
			echoheader();
			echohtmlstart('Редактирование группы');
...
HTML;
		...
		msgbox('Ошибка', 'Сообщество не найдено', '?mod=clubs');
...
$sql_ = $db->super_query("SELECT tb1.id, title, date, traf, real_admin, del, ban, rec_num, tb2.user_name FROM `".PREFIX."_clubs` tb1, `".PREFIX."_users` tb2 WHERE tb1.real_admin = tb2.user_id {$where_sql} ORDER by {$order_sql} LIMIT {$limit_page}, {$gcount}", 1);

//Кол-во людей считаем
$numRows = $db->super_query("SELECT COUNT(*) AS cnt FROM `".PREFIX."_clubs` WHERE id != '' {$where_sql}");
...
<input type="hidden" name="mod" value="clubs" />
...
<div style="background:#fff;float:left;padding:5px;width:243px;text-align:center;margin-left:1px"><a href="?mod=clubs&act=edit&id={$row['id']}" title="Записей на стене: {$row['rec_num']}" style="{$color}">{$row['title']}</a></div>
...
<form action="?mod=massaction&act=clubs" method="post" name="edit">
