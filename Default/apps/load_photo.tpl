...
$(document).ready(function(){
	Xajax = new AjaxUpload('upload', {
		action: '/index.php?go=editapp&page=upload&act={id}',
		name: 'uploadfile',
		...
		onComplete: function (file, response) {
			if(response == 'bad_format')
				...
				Box.Close('photo');
				var img = document.getElementById('app_photo');
				img.src = response;
...
<div class="load_photo_quote">Вы можете загрузить сюда фотографию только приложения. Поддерживаются форматы JPG, PNG и GIF.</div>
