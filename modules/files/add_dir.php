...
// Проверяем количество папок
    ...
    $count_dir = DB :: $dbh -> querySingle("SELECT count(*) FROM `files_dir` WHERE `dir`=? AND `user`=?;", array(0, $user['id']));
...
// Добавляем папку в базу
    ...
    DB :: $dbh -> query("INSERT INTO `files_dir` (`name`, `description`, `access`, `user`, `time`) VALUES (?, ?, ?, ?, ?);", array($name, $description, $access, $user['id'], time()));	

// Уведомляем

    $system->redirect("Папка успешно добавлена", "/modules/files/".$user['id']."");	
