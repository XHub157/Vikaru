...
// Получаем информацию о файле
    ...
    $file = DB :: $dbh -> queryFetch("SELECT `user` FROM `files` WHERE `id`=? LIMIT 1;", array($act['file']));	
	
// Проверяем права

    if ($act['user'] == $user['id'] || $file['user'] == $user['id'] || $user['access'] > 0 && $user['access'] < 5) {
...
// Удаляем лайки
    ...
    DB :: $dbh -> query("DELETE FROM `files_comments_like` WHERE `comments`=? LIMIT 1;", array($act['id']));		

// Удаляем из базы
	
    DB :: $dbh -> query("DELETE FROM `files_comments` WHERE `id`=? LIMIT 1;", array($act['id']));	
...
// Уведомляем
    ...
    $system->redirect("Комментарий успешно удалён", "/modules/files/file/".$act['file']."");
