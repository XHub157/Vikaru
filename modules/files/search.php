...
// Подчёт количества файлав

    $count = DB :: $dbh -> querySingle("SELECT count(*) FROM `files` WHERE `description` like '%".$search."%';");	
...
// Выводим результаты в блок
    ...
    echo '<div class="hide">Результатов поиска <span style="font-weight: bold;">'.$search.'</span> найдено '.$count.' файлов</div>';

// Выводим новости

    $q = DB :: $dbh -> query("SELECT * FROM `files` WHERE `description` like '%".$search."%' ORDER BY `time` DESC  LIMIT " . $page . ", " . $config['post'] . ";");
...
// Выводим навигацию
    ...
    $navigation->pages('/modules/files/search/?', $config['post'], $page, $count);	 	
...
// Выводим ошибки
    ...
    } else { $system->redirect("Отказано в доступе", "".(empty($user) ? '/' : '/modules/files/'.$user['id'].'').""); }	
