...
// Антифлуд
    ...
    $antiflood = DB :: $dbh -> querySingle("SELECT count(*) FROM `gifts_dir` WHERE `edit_time`>? AND `id`=?;", array(time()-$config['antiflood_edit'], $act['id']));
...
// Выполняем запрос в базу
    ...
    DB :: $dbh -> query("UPDATE `gifts_dir` SET `name`=?, `edit`=?, `edit_time`=? WHERE `id`=? LIMIT 1;", array($name, $user['id'], time(), $act['id']));	
	
// Уведомляем

    $system->redirect("Категория успешно отредактирована", "/modules/gifts/dir/".$act['id']."");
