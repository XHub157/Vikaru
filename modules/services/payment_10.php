...
// Пополняем баланс	
    ...
    DB :: $dbh -> query("UPDATE `user` SET `money`=`money`+10 WHERE `id`=? LIMIT 1;", array($user['id']));
	
// Запись в логи

    $message = 'Пополнение внутреннего баланса';
    DB :: $dbh -> query("INSERT INTO `user_logs` (`section`, `user`, `message`, `price`, `money`, `ip`, `ua`, `time`) VALUES (?, ?, ?, ?, ?, ?, ?, ?);", array(1, $user['id'], $message, 10, $user['money'] + 10, $system->ip(),  $system->ua(), time()));

// Уведомляем

    $system->redirect("Ваш баланс успешно пополнен на 10 монет", "/modules/services/");	
