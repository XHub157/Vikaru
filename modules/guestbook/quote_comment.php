...
// Запрос в базу	
    ...
    DB :: $dbh -> query("INSERT INTO `guestbook_comments` (`profile`, `user`, `answer`, `comment`, `time`, `ip`, `ua`) VALUES (?, ?, ?, ?, ?, ?, ?);", array($act['profile'], $user['id'], $act['user'], $quote, time(), $system->ip(), $system->ua()));	
...
// Выводим форму
    ...
    echo $system->form('/modules/guestbook/quote_comment/'.$act['id'].'', '', 'Цитировать', 'Комментарий', '10000', 'comment', '', ''.$user['sid'].'', 'comment'); 	
