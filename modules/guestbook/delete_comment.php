...
// Удаляем лайк
    ...
    DB :: $dbh -> query("DELETE FROM `guestbook_comments_like` WHERE `comments`=? LIMIT 1;", array($act['id']));	

// Удаляем из базы
	
    DB :: $dbh -> query("DELETE FROM `guestbook_comments` WHERE `id`=? LIMIT 1;", array($act['id']));	

// Уведомляем

    $system->redirect("Комментарий успешно удалён", "/modules/guestbook/".$act['profile']."");
