...
// Подсчёт сообщений
    ...
    $count = DB :: $dbh -> querySingle("SELECT count(*) FROM `mail_favorites` WHERE `user`=?;", array($user['id']));
...
// Выводим сообщения	
    ...
    $q = DB :: $dbh -> query("SELECT * FROM `mail_favorites` WHERE `user`=? ORDER BY `time` DESC  LIMIT " . $page . ", " . $config['post'] . ";", array($user['id']));
...
// Выводим навигацию
    ...
    $navigation->pages('/modules/mail/favorites/?', $config['post'], $page, $count);	 	
