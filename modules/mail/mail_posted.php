...
<input type="submit" style="line-height: 19px;  margin-top: 0;" class="main_submit" value="Найти">
    ...
    <a href="/modules/mail/posted" class="listbar-act">Отправленные</a>
    </div>
';
	
// Подсчёт сообщений

    $count = DB :: $dbh -> querySingle("SELECT count(*) FROM `mail_message` WHERE `user`=?;", array($user['id']));
	
    if ($count > 0) {
    if ($page >= $count) {
    $page = 0; } $i = 0;

// Выводим список отправленных сообщений
...
// Выводим сообщения	
    ...
    $q = DB :: $dbh -> query("SELECT * FROM `mail_message` WHERE `user`=? ORDER BY `time` DESC  LIMIT " . $page . ", " . $config['post'] . ";", array($user['id']));
...
// Выводим сообщение
    ...
    <a href="/modules/mail/contact/'.$act['profile'].'">
...
// Выводим навигацию
    ...
    $navigation->pages('/modules/mail/posted/?', $config['post'], $page, $count);	 	
