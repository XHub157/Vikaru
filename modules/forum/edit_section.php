...
// Антифлуд
    ...
    $antiflood = DB :: $dbh -> querySingle("SELECT count(*) FROM `forum_section` WHERE `edit_time`>? AND `id`=?;", array(time()-$config['antiflood_edit'], $act['id']));
...
// Выполняем запрос в базу
    ...
    DB :: $dbh -> query("UPDATE `forum_section` SET `name`=?, `description`=?, `edit`=?, `edit_time`=? WHERE `id`=? LIMIT 1;", array($name, $description, $user['id'], time(), $act['id']));	
	
// Уведомляем

    $system->redirect("Раздел успешно отредактирован", "/modules/forum/section/".$act['id']."");
