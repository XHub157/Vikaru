...
// Запрещяем удаление комментария от системы
...
// Удаляем лайк
    ...
    DB :: $dbh -> query("DELETE FROM `forum_comments_like` WHERE `comments`=? LIMIT 1;", array($act['id']));	

// Удаляем из базы
	
    DB :: $dbh -> query("DELETE FROM `forum_comments` WHERE `id`=? LIMIT 1;", array($act['id']));	
...
// Уведомляем
    ...
    $system->redirect("Комментарий успешно удалён", "/modules/forum/topic/".$act['topic']."");
