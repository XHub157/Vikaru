...
// Антифлуд
    ...
    $antiflood = DB :: $dbh -> querySingle("SELECT count(*) FROM `forum_comments` WHERE `edit_time`>? AND `id`=?;", array(time()-$config['antiflood_edit'], $act['id']));

// Только если не было изменений в течении $config['antiflood_edit'] секунд
	
    if (empty($antiflood)) {		
	
// Запрещяем редактировать сообщения от системы
...
// Выполняем запрос в базу
    ...
    DB :: $dbh -> query("UPDATE `forum_comments` SET `comment`=?, `edit`=?, `edit_time`=? WHERE `id`=? LIMIT 1;", array($comment, $user['id'], time(), $act['id']));	
	
// Уведомляем

    $system->redirect("Комментарий успешно отредактирован", "/modules/forum/topic/".$act['topic']."");
...
// Выводим форму
    ...
    echo $system->form('/modules/forum/edit_comment/'.$act['id'].'', ''.$act['comment'].'', 'Редактировать', 'Комментарий', '10000', 'comment', '', ''.$user['sid'].'', 'comment'); 
