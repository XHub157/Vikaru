...
// Добавляем дневник в базу
    ...
    DB :: $dbh -> query("INSERT INTO `diary` (`name`, `description`, `access`, `comment`, `censored`, `user`, `time`, `ip`, `ua`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);", array($name, $description, $access, $comment, $censored,  $user['id'], time(), $system->ip(), $system->ua()));
