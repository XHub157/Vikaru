...
// Антифлуд
    ...
    $antiflood = DB :: $dbh -> querySingle("SELECT count(*) FROM `diary_comments` WHERE `edit_time`>? AND `id`=?;", array(time()-$config['antiflood_edit'], $act['id']));
...
// Выполняем запрос в базу
    ...
    DB :: $dbh -> query("UPDATE `diary_comments` SET `comment`=?, `edit`=?, `edit_time`=? WHERE `id`=? LIMIT 1;", array($comment, $user['id'], time(), $act['id']));	
	
// Уведомляем

    $system->redirect("Комментарий успешно отредактирован", "/modules/diary/".$act['diary']."");
...
// Выводим форму
    ...
    echo $system->form('/modules/diary/edit_comment/'.$act['id'].'', ''.$act['comment'].'', 'Редактировать', 'Комментарий', '10000', 'comment', '', ''.$user['sid'].'', 'comment'); 
