...
// Только если был создан дневник в течении 30 секунд
...
// Только дневники с открытым доступом
...
// Добавляем дневник в базу
    ...
    DB :: $dbh -> query("INSERT INTO `diary` (`name`, `description`, `share`, `section`, `access`, `comment`, `censored`, `user`, `time`, `ip`, `ua`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);", array($name, $description, $act['id'], 0, $access, $comment, $censored,  $user['id'], time(), $system->ip(), $system->ua()));
...
// Выводим ошибки
    ...
    header ("Location: /modules/diary/user/$user[id]");  
    }
...
// Выводим блок
    ...
    <input type="radio" class="middle" name="comment" value="1" /> Моим друзьям <br /> 
    <input type="radio" class="middle" name="comment" value="2"/> Только мне <br />
    </div>	
    <div class="block">
