...
// Проверяем права доступа
    ...
    echo '
    <div class="hide">
    Список посмотревших дневник <a href="/modules/diary/'.$diary['id'].'">'.$diary['name'].'</a>
    </div>
    ';

// Подчёт количеста просмотревших	
	
    $count = DB :: $dbh -> querySingle("SELECT count(*) FROM `diary_view` WHERE `diary`=?;", array($diary['id']));

    if ($count > 0) {
    if ($page >= $count) {
    $page = 0; } $i = 0;

    $q = DB :: $dbh -> query("SELECT * FROM `diary_view` WHERE `diary`=? ORDER BY `time` DESC  LIMIT " . $page . ", " . $config['post'] . ";", array($diary['id']));   
...
// Выводим навигацию
    ...
    $navigation->pages('/modules/diary/views/'.$diary['id'].'/?', $config['post'], $page, $count);		
