...
// Удаляем лайки
    ...
    DB :: $dbh -> query("DELETE FROM `arbor_comments_like` WHERE `comments`=? LIMIT 1;", array($act['id']));

// Удаляем из базы
	
    DB :: $dbh -> query("DELETE FROM `arbor` WHERE `id`=? LIMIT 1;", array($act['id']));		

// Уведомляем

    $system->redirect("Комментарий успешно удалён", "/modules/arbor/");	
