...
// Антифлуд
    ...
    $antiflood = DB :: $dbh -> querySingle("SELECT count(*) FROM `arbor` WHERE `edit_time`>? AND `id`=?;", array(time()-$config['antiflood_edit'], $act['id']));
...
// Выполняем запрос в базу
    ...
    DB :: $dbh -> query("UPDATE `arbor` SET `comment`=?, `edit`=?, `edit_time`=? WHERE `id`=? LIMIT 1;", array($comment, $user['id'], time(), $act['id']));

// Уведомляем

    $system->redirect("Комментарий успешно отредактирован", "/modules/arbor/");
...
// Выводим форму
    ...
    echo $system->form('/modules/arbor/edit_comment/'.$act['id'].'', ''.$act['comment'].'', 'Редактировать', 'Комментарий', '10000', 'comment', '', ''.$user['sid'].'', 'comment'); 
