...
";
    ...
    <a href="/modules/dating/women" class="listbar-act">Женщины</a>
...
// Подсчёт девушек
    ...
    $count = DB :: $dbh -> querySingle("SELECT count(*) FROM `user` WHERE `dating`=? AND `sex`=?;", array(0, 1));
...
// Выводим девушек
    ...
    $q = DB :: $dbh -> query("SELECT * FROM `user` WHERE `dating`=? AND `sex`=? ORDER BY `rating` DESC  LIMIT " . $page . ", " . $config['post'] . ";", array(0, 1));	
...
// Выводим навигацию
    ...
    $navigation->pages('/modules/dating/women/?', $config['post'], $page, $count);	
	 
// Выводим ошибки	
	
    } else { $system->show("Девушек нет"); } 
