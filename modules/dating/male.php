...
";
	...
	<a href="/modules/dating/male" class="listbar-act">Мужчины</a>
...
// Подсчёт парней
    ...
    $count = DB :: $dbh -> querySingle("SELECT count(*) FROM `user` WHERE `dating`=? AND `sex`=?;", array(0, 0));
...
// Выводим парней
    ...
    $q = DB :: $dbh -> query("SELECT * FROM `user` WHERE `dating`=? AND `sex`=? ORDER BY `rating` DESC  LIMIT " . $page . ", " . $config['post'] . ";", array(0, 0));	
...
// Выводим навигацию
    ...
    $navigation->pages('/modules/dating/male/?', $config['post'], $page, $count);	
	 
// Выводим ошибки	
	
    } else { $system->show("Парней нет"); } 
