...
// Добавляем фотоальбом в базу
    ...
    DB :: $dbh -> query("INSERT INTO `photo` (`name`, `description`, `censored`, `user`, `time`, `ip`, `ua`, `key`, `type`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?);", array($name, $description, $censored, $user['id'], time(), $system->ip(), $system->ua(), $key, $type));	
...
// Уведомляем
    ...
    $system->redirect("Фото успешно добавлено", "/modules/photo_album/".$user['id']."");	
...
// Выводим блок
    ...
    <a href="/modules/photo_album/'.$user['id'].'">'.$user['login'].'</a> 
    </div>
    <div class="block">
