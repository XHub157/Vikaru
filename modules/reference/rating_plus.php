...
// Выполняем запрос в базу
    ...
    DB :: $dbh -> query("UPDATE `reference_rating` SET `section`=?, `time`=? WHERE `user`=? AND `reference`=? LIMIT 1;", array(1, time(), $user['id'], $act['id']));	
...
// Делаем запрос в базу
    ...
    DB :: $dbh -> query("INSERT INTO `reference_rating` (`user`, `time`, `reference`, `section`) VALUES (?, ?, ?, ?);", array($user['id'], time(), $act['id'], 1));
