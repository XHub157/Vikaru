    ...
    if(item){
      item.dataIndex++;
        if(previousPoint != item.dataIndex){
            ...
            if(!y)
              showTooltip(item.pageX, item.pageY, '<b>Ќе было посетителей</b><br /><small>' + item.dataIndex + ' ' + $('#tek_month').val() +'</small>');
            else
			...
			langNumric( 'gram_num'+item.dataIndex, y, 'уникальный посетитель', 'уникальных посетител€', 'уникальных посетителей', 'уникальных посетителей' );  
        }
    } else {
        $('#tooltip_1').remove();
        previousPoint = null;
    ...
    if(item){
      item.dataIndex++;
        if(previousPoint2 != item.dataIndex){
          ...
          var x = item.datapoint[0], y = item.datapoint[1];
          if(!y)
            showTooltip(item.pageX, item.pageY, '<b>Ќе было просмотров</b><br /><small>' + item.dataIndex + ' ' + $('#tek_month').val() +'</small>');
          else
...
<div class="search_form_tab" style="margin-top:-9px">
 <div class="buttonsprofile albumsbuttonsprofile buttonsprofileSecond" style="height:22px">
  <a href="/u{uid}"><div><b>  моей странице</b></div></a>
  <div class="buttonsprofileSec"><a href="/my_stats"><div><b>—татистика страницы</b></div></a></div>
  <div class="fl_r">
   ...
   <div class="button_div fl_l"><button onClick="location.href = '/?go=my_stats&m='+$('#month').val()+'&y='+$('#year').val()">ѕросмотреть</button></div>
  </div>
 </div>
</div>
<div class="clear"></div>
<input type="hidden" id="tek_month" value="{t-date}" />
<div class="margin_top_10"></div><div class="allbar_title" style="font-size:13px">”никальные посетители</div>
...
<div class="margin_top_10"></div><div class="allbar_title" style="font-size:13px;color:#bf68a6">ѕросмотры</div>
<div id="graph-wrapper">
 <div class="graph-container">
