...
var apps = {
	...
	view: function(a, h, c){
		...
		$.post('/index.php?go=apps&act=view', {id: a}, function(d){
			$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 607px;margin-top:30px;"><div class="news_title fl_l" style="color: #2B587A;font-weight: bold;margin-top:-5px">Запуск приложения</div><a class="cursor_pointer fl_r" onclick="apps.close(\'/apps\')" style="margin-top:-5px">Закрыть</a><div class="clear"></div>'+d+'</div></div></div>');
		});
	},
	mydel: function(i, t){
		if(t){
	...
	LoadPhoto: function(id){
		Page.Loading('start');
		$.get('/index.php?go=editapp&page=load_photo&act='+id, function(data){
			Box.Show('photo', 400, 'Загрузка фотографии для приложения', data, lang_box_canсel);
			Page.Loading('stop');
		});
	}
}
...
var Photo = {
	...
	like: function(i){
		...
		if(rnum < 10) $('#photos_likes_but'+i).css('width', '197px');
		else if(rnum < 100) $('#photos_likes_but'+i).css('width', '207px');
		else if(rnum < 1000) $('#photos_likes_but'+i).css('width', '217px');
		else if(rnum < 10000) $('#photos_likes_but'+i).css('width', '229px');
		else if(rnum < 100000) $('#photos_likes_but'+i).css('width', '239px');
	...
	dislike: function(i){
		...
		if(rnum <= 0) $('#photos_likes_but'+i).css('width', '177px');
		else if(rnum < 10) $('#photos_likes_but'+i).css('width', '197px');
		else if(rnum < 100) $('#photos_likes_but'+i).css('width', '207px');
		else if(rnum < 1000) $('#photos_likes_but'+i).css('width', '217px');
		else if(rnum < 10000) $('#photos_likes_but'+i).css('width', '229px');
		else if(rnum < 100000) $('#photos_likes_but'+i).css('width', '239px');
...
var friends = {
	...
	myreject: function(myreject_user_id){
		Page.Loading('start');
		$.get('/friedns/myreject/'+myreject_user_id, function(data){
			Page.Loading('stop');
			$('#action_'+myreject_user_id).html(lang_take_no2);
		});
	},
...
var messages = {
	new_: function(user_id){
		var content = '<style>.box_conetnt{background:#f0f0f0}</style><div style="padding:20px">'+
		'<h7>Тема</h7><div class="clear"></div><br><input type="text" id="theme" class="inpst" maxlength="255" style="width:410px" /><div class="mgclr"></div>'+
		'<br><h7>Сообщение</h7><div class="clear"></div><br><textarea id="msg" class="inpst" style="width:410px;height:120px;"></textarea>'+
		'</div>';
...
fall: function(uid, page_num, c){ 
  ...
  Box.Page('/index.php?go=subscriptions&act=fall', 'uid='+uid+page, 'subscriptions_'+page_num, 525, 'Подписчики', lang_msg_close, 0, 0, 345, 1, 1, 1, 0, cache);     
...
var wall = {
	...
	send_w: function(){
		...
		attach_files = $('#vaLattach_files').val();
		for_user_id = location.href.split('http://'+location.host+'/wall');
	...
	all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=wall&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 625, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	...
	attach_addgraffiti:function(id){
		Box.Page('/index.php?go=graffiti&id='+id+'&act=add', '', '', 600, 'Граффити на стену', lang_box_canсel, 0, 0, 0, 1, 0, 1, 0, 0);
	},
	attach_addDoc: function(){
		Box.Page('/index.php?go=doc', '', 'all_doc', 627, 'Прикрепление документа', lang_box_canсel, 0, 0, 'auto', 0, 0, 1, 0, 0);
	},
	tell: function(id){
		$('#wall_tell_'+id).hide();
		myhtml.title_close(id);
	...
	if(is_moz && !is_chrome) scrollTopForFirefox = $(window).scrollTop();
		$('html').css('overflow', 'hidden');
		if(is_moz && !is_chrome) $(window).scrollTop(scrollTopForFirefox);
	$.post('/index.php?go=wall&act=zw', function(body){
		$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 540px;padding:15px;"><div class="news_title fl_l" style="color: #2B587A;font-weight: bold;">'+name+'</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div class="clear"></div><br>'+body+'</div></div></div>');
	});
	}
}
...
var gStatus = {
	...
	startTell: function(){
		...
		if(tell_friends){
			if(text != 0){
				$.post('/index.php?go=wall&act=send', {wall_text: wall_text, for_user_id: for_user_id[1]}, function(data){
					$('#wall_records').html(data);
					$('#wall_all_record').html('');
...
var settings = {
	...
	logs:function(){
		$.post('/index.php?go=settings&act=userlogs',function(body){
			$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 540px;padding:15px;"><div class="miniature_title fl_l">История активности</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div class="clear"></div>'+body+'</div></div></div>');
		});
	},
	elfocus: function(id) {
		$('#alias').focus();
	...
	event: function(event){
		...
		if(oi != 'privacyMenu_msg' && oi != 'privacy_lnk_msg' && oi != 'privacyMenu_wall1' && oi != 'privacy_lnk_wall1' && oi != 'privacyMenu_wall2' && oi != 'privacy_lnk_wall2' && oi != 'privacyMenu_wall3' && oi != 'privacy_lnk_wall3' && oi != 'privacyMenu_info' && oi != 'privacy_lnk_info' && oi != 'privacyMenu_guests1' && oi != 'privacy_lnk_guests1' && oi != 'privacyMenu_guests2' && oi != 'privacy_lnk_guests2' && oi != 'privacyMenu_gift' && oi != 'privacy_lnk_gift' && oi != 'privacyMenu_audio' && oi != 'privacy_lnk_audio' && oi != 'privacyMenu_video' && oi != 'privacy_lnk_video' && oi != 'privacyMenu_group' && oi != 'privacy_lnk_group' && oi != 'privacyMenu_notes' && oi != 'privacy_lnk_notes' && oi != 'privacyMenu_public' && oi != 'privacy_lnk_public')
			$('#privacyMenu_msg, #privacyMenu_wall1, #privacyMenu_wall2, #privacyMenu_wall3, #privacyMenu_info, #privacyMenu_guests1, #privacyMenu_guests2, #privacyMenu_gift, #privacyMenu_audio, #privacyMenu_video, #privacyMenu_group, #privacyMenu_notes, #privacyMenu_public').fadeOut(120);
	},
	savePrivacy: function(){
		...
		var val_gift = $('#val_gift').val();
		var val_audio = $('#val_audio').val();
		var val_video = $('#val_video').val();
		var val_group = $('#val_group').val();
		var val_notes = $('#val_notes').val();
		var val_public = $('#val_public').val();
		butloading('savePrivacy', 55, 'disabled');
		$.post('/index.php?go=settings&act=saveprivacy', {val_msg: val_msg, val_wall1: val_wall1, val_wall2: val_wall2, val_wall3: val_wall3, val_info: val_info, val_guests1: val_guests1, val_guests2: val_guests2, val_gift: val_gift,val_audio: val_audio,val_video: val_video,val_group: val_group,val_notes: val_notes,val_public: val_public}, function(){
			$('#ok_update').show();
	...
	addblacklist: function(bad_user_id){
		...
		$.post('/index.php?go=settings&act=addblacklist', {bad_user_id: bad_user_id}, function(){
			...
			$('#text_add_blacklist').text('Удалить из черного списка');
	...
	delblacklist: function(bad_user_id, type){
		...
		$.post('/index.php?go=settings&act=delblacklist', {bad_user_id: bad_user_id}, function(){
			if(type){
				...
				$('#text_add_blacklist').text('Добавить в черный список');
				$('#addblacklist_load').hide();
			} else {
...
//GIFTS
var gifts = {
	box: function(user_id, c, j){
		if(!j) j = 0;
		if(!j) viiBox.start();
		$.post('/index.php?go=gifts&act=view', {user_id: user_id, section: j}, function(d){
			if(!j) viiBox.win('gifts_box', d);
			else {
				var type = $('#type').val();
				$('#tb_box_'+type).hide();
				$('#tb_box_'+j).show().html('<div class="gifts_section clear_fix" id="gifts_box">'+d+'</div>');
				$('#selection_'+type).removeClass('summary_tab_sel').addClass('summary_tab');
				$('#selection_'+j).addClass('summary_tab_sel').removeClass('summary_tab');
				$('#type').val(j);
			}
		})
	},
	...
	select: function(gid, fid){
		viiBox.clos('gifts_box', 1);
		Box.Close(0, 1);
		viiBox.start();
		viiBox.win('show_box_send', '<div class="miniature_box"><div class="miniature_pos" style="width: 450px;padding:0px;"><a class="fl_r tb_close" onclick="viiBox.clos(\'show_box_send\', 1)" style="cursor: pointer;">Закрыть</a><div class="tb_title" id="tb_title">Отправка подарка</div><div id="tb_box_1"><div class="box_conetnt" id="box_content_send_gift378" style="height: 438px;border-bottom: 0px solid #D7D7D7;"><center style="margin-bottom: 20px;"><img src="/uploads/gifts/'+gid+'.jpg" style="margin-top:30px"></center><b style="color: #36638E;margin: 0px 40px;">Ваше сообщение</b><textarea id="msgfgift'+gid+'" class="inpst" style="width: 350px;height: 50px;margin: 10px 40px;"></textarea><div class="html_checkbox" id="private_gifts'+gid+'" onclick="myhtml.checkbox(this.id);" style="margin: 0px 40px;color: #777;">Показывать моё имя только получателю</div></div><div class="box_controls_wrap"><div class="box_controls"><table cellspacing="0" cellpadding="0" class="fl_r"><tbody><tr><td><div class="button_blue"><button onclick="gifts.send('+gid+', '+fid+')" id="box_butt_create">Отправить</button></div></td><td><div class="button_gray"><button onclick="viiBox.clos(\'show_box_send\', 1)">Отмена</button></div></td></tr></tbody></table></div></div></div></div></div>');
	},
	send: function(gfid, fid){
		viiBox.clos('show_box_send', 1);
		var privacy = $('#private_gifts'+gfid).val();
	...
	showGiftPrice: function(j,text){
		$('#gift_'+j).append('<div id="gift_price"><span id="gift_price_str">'+text+'</span><b class="rbottom"><b class="r"></b></b></div>');
	},
...
var clubs = {
	createbox: function(){
		Box.Show('create', 490, 'Создание новой группы', '<div style="padding:20px"><div class="videos_text">Название</div><input type="text" class="videos_input" id="title" maxlength="65" /></div>', lang_box_canсel, 'Создать группу', 'clubs.creat()', 100, 0, 0, 0, 0, 'title');
		$('#title').focus();
	},
	creat: function(){
		var title = $('#title').val();
		if(title != 0){
			...
			$.post('/index.php?go=clubs&act=send', {title: title}, function(id){
				Box.Close();
				Page.Go('/club'+id);
			});
		} else
	...
	exit: function(id){
		$('#exitlink'+id).html('<div class="color777" style="margin-top:6px;margin-right:7px">Вы вышли из группы.</div>');
		$.post('/index.php?go=clubs&act=exit', {id: id});
	},
	...
	setPrivacy: function(val_id, mtext, opt, text_id){
		...
		$('#'+text_id).text(mtext);
		$('#selected_p_'+text_id).text(mtext);
		clubs.privacyClose(val_id);
	},
	event: function(event){
		var oi = (event.target) ? event.target.id: ((event.srcElement) ? event.srcElement.id : null);
		var fast_oi = oi.substring(0, 9);

		if(oi != 'privacyMenu_wall1' && oi != 'privacyMenu_intog' && oi != 'privacyMenu_board' && oi != 'wall_lnk_wall1' && oi != 'intog_lnk_wall1'&& oi != 'board_lnk_wall1')
			$('#privacyMenu_wall1','#privacyMenu_intog','#privacyMenu_board').fadeOut(120);
	},
	login: function(id){
		$('#yes').hide();
		$('#no-fev').fadeIn('fast');
		$.post('/index.php?go=clubs&act=login', {id: id});
	},
	loadphoto: function(id){
		Box.Page('/index.php?go=clubs&act=loadphoto_page', 'id='+id, 'loadphoto', 400, lang_title_load_photo, lang_box_canсel, 0, 0, 0, 0, 0, 0, 0, 1);
	},
	delphoto: function(id){
		Box.Show('del_photo', 400, lang_title_del_photo, '<div style="padding:15px;">'+lang_del_photo+'</div>', lang_box_canсel, lang_box_yes, 'clubs.startdelete('+id+')');
	},
	startdelete: function(id){
		$('#box_loading').show();
		ge('box_butt_create').disabled = true;
		$.post('/index.php?go=clubs&act=delphoto', {id: id}, function(){
	...
	addcontact: function(id){
		Box.Page('/index.php?go=clubs&act=addfeedback_pg', 'id='+id, 'addfeedback', 400, 'Добавление контактного лица', lang_box_canсel, 'Сохранить', 'clubs.savefeedback('+id+')', 0, 0, 0, 0, 'upage', 0);
	},
	savefeedback: function(id){
		...
		if($('#feedimg').attr('src') != template_dir+'/images/contact_info.png'){
			$('#box_loading').show();
			ge('box_butt_create').disabled = true;
			$.post('/index.php?go=clubs&act=addfeedback_db', {id: id, upage: upage, office: office, phone: phone, email: email}, function(d){
				if(d == 1){
	...
	allfeedbacklist: function(id){
		Box.Page('/index.php?go=clubs&act=allfeedbacklist', 'id='+id, 'allfeedbacklist', 450, 'Контакты', 'Закрыть', 0, 0, 300, 1, 1, 1, 0, 0);
	},
	delfeedback: function(id, uid){
		...
		updateNum('#fnumu');
		if(si <= 0){
			$('#feddbackusers').html('<div class="line_height color777" align="center">Страницы представителей, номера телефонов, e-mail<br /><a href="/club'+id+'" onClick="clubs.addcontact('+id+'); return false">Добавить контакты</a></div>');
		...
		$.post('/index.php?go=clubs&act=delfeedback', {id: id, uid: uid});
	},
	...
	editfeeddave: function(id, uid){
		...
		$.post('/index.php?go=clubs&act=editfeeddave', {id: id, uid: uid, office: office, phone: phone, email: email});
	},
	checkFeedUser: function(){
		var upage = $('#upage').val();
		var pattern = new RegExp(/^[0-9]+$/);
		if(pattern.test(upage)){
			$.post('/index.php?go=clubs&act=checkFeedUser', {id: upage}, function(d){
				d = d.split('|');
				if(d[0]){
					if(d[1])
	...
	saveinfo: function(id){
		...
		if(website != 0){
			$('#website_display').show();
			$('#e_website').html("<a href="+website+">"+website+"</a>");
		}
		var val_wall1 = $('#wall1').val();
		var val_intog = $('#intog').val();
		var val_board = $('#board').val();
		if(!adres_page)	var adres_page = 'public'+id;
		var pattern = new RegExp(/^[a-zA-Z0-9_-]+$/);
		if(pattern.test(adres_page)){
			butloading('pubInfoSave', 55, 'disabled');
			$.post('/index.php?go=clubs&act=saveinfo', {id: id, title: title, descr: descr, website:website, comments: comments, adres_page: adres_page, val_wall1: val_wall1, val_board: val_board, val_intog: val_intog}, function(d){
				if(d == 'err_adres')
	...
	delpublic: function(id){
			butloading('pubDel', 55, 'disabled');
			$.post('/index.php?go=clubs&act=delpublic', {id: id}, function(d){
				if(d == 'delete_ok')
					Box.Info('err', 'Успешно', 'Страница удалена!', 130, 1500);
				
			butloading('pubDel', 55, 'enabled', 'Удалить страницу');
			Box.Close('del_club');
			});
	},
	okfevs: function(id,fev){
		$.post('/index.php?go=clubs&act=okfevs', {id: id, user_fev: fev}, function(){});
		$('#admin'+fev).hide();
	},
	nofevs: function(id,fev){
		$.post('/index.php?go=clubs&act=notfevs', {id: id, user_fev: fev}, function(){});
		$('#admin'+fev).hide();
	},
	delclubbox: function(id){
		Box.Show('del_club', 400, lang_title_del_photo, '<div style="padding:15px;">Вы уверены, что хотите удалить группу?</div>', lang_box_canсel, lang_box_yes, 'clubs.delpublic('+id+')');
	},
	overspublic: function(id){
			butloading('pubOvers', 55, 'disabled');
			$.post('/index.php?go=clubs&act=overspublic', {id: id}, function(d){
				if(d == 'delete_ok')
					Box.Info('err', 'Успешно', 'Страница восстановлена!', 130, 1500);
				else if(d == 'not_time_unbanned')
					Box.Info('err', 'Ошибка', 'Время для восстановления закончилось, обратитесь к администрации!', 130, 1500);
				else if(d == 'not_banned')
					Box.Info('err', 'Ошибка', 'Страница не заблокирована!', 130, 1500);
				
				
			butloading('pubOvers', 55, 'enabled', 'Восстановить страницу');
			});
	},
	editform: function(){
	...
	edittab_admin: function(id){
		$('#edittab2').slideDown('slow');
		$('#edittab3').hide();
		$('#edittab1').hide();
		$('#public_editbg_container').animate({scrollLeft: "+1120"});
	},
	edittab_fev: function(id){
		$('#edittab3').slideDown('slow');
		$('#edittab2').hide();
		$('#edittab1').hide();
		$('#public_editbg_container').animate({scrollLeft: "+1680"});
	},
	addadmin: function(id){
		...
		if(new_admin_id && !check_adm){
			Box.Page('/index.php?go=clubs&act=new_admin', 'new_admin_id='+new_admin_id, 'new_admin_id', 400, 'Назначение руководителя', 'Закрыть', 'Назначить руководителем', 'clubs.send_new_admin('+id+', '+new_admin_id+')', 130, 0, 0, 0, 0, 0);
		} else
	...
	send_new_admin: function(id, new_admin_id){
		...
		var data = '<div class="public_oneadmin" id="admin'+new_admin_id+'"><a href="/u'+new_admin_id+'" onClick="Page.Go(this.href); return false"><img src="'+ava+'" align="left" width="32" /></a><a href="/u'+new_admin_id+'" onClick="Page.Go(this.href); return false">'+adm_name+'</a><br /><a href="/" onClick="clubs.deladmin(\''+id+'\', \''+new_admin_id+'\'); return false"><small>Удалить</small></a></div>';
		$('#admins_tab').append(data);
		Box.Close();
		$('#new_admin_id').val('');
		$.post('/index.php?go=clubs&act=send_new_admin', {id: id, new_admin_id: new_admin_id});
	},
	deladmin: function(id, uid){
		$('#admin'+uid).remove();
		$.post('/index.php?go=clubs&act=deladmin', {id: id, uid: uid});
	},
	...
	wall_send_comm: function(rec_id, public_id){
		...
		if(wall_text != 0){
			...
			$.post('/index.php?go=clubs&act=wall_send_comm', {rec_id: rec_id, wall_text: wall_text, public_id: public_id}, function(data){
	...
	wall_delet: function(rec_id){
		...
		updateNum('#rec_num');
		$.post('/index.php?go=clubs&act=wall_del', {rec_id: rec_id});
	},
	comm_wall_delet: function(rec_id, public_id){
		...
		$.post('/index.php?go=clubs&act=wall_del', {rec_id: rec_id, public_id: public_id});
	},
	wall_all_comments: function(rec_id, public_id){
		textLoad('wall_all_comm_but_'+rec_id);
		$('#wall_all_but_link_'+rec_id).attr('onClick', '');
		$.post('/index.php?go=clubs&act=all_comm', {rec_id: rec_id, public_id: public_id}, function(data){
	...
	wall_page: function(){
		...
		if($('#load_wall_all_records').text() == 'к предыдущим записям' && $('#rec_num').text() > 10){
			...
			$.post('/index.php?go=club&gid='+public_id, {page_cnt: page_cnt}, function(data){
				$('#public_wall_records').append(data);
				$('#page_cnt').val((parseInt($('#page_cnt').val())+1));
				if($('.wallrecord').size() == $('#rec_num').text()){
					$('#wall_all_records').hide();
				} else {
					$('#wall_all_records').attr('onClick', 'clubs.wall_page(\''+public_id+'\')');
					$('#load_wall_all_records').html('к предыдущим записям');
				}
			});
		}
	...
	wall_attach_addphoto: function(id, page_num, public_id){
		...
		Box.Page('/index.php?go=clubs&act=photos', 'public_id='+public_id+page, 'c_all_photos_'+page_num, 627, lang_wall_attatch_photos, lang_box_canсel, 0, 0, 400, 1, 0, 1, 0, 1);
	},
	...
	wall_photo_view: function(rec_id, public_id, src, pos){
		...
		if(size == 1){
			...
			var next = 'clubs.wall_photo_view_next('+rec_id+'); return false';
		}
		
		var content = '<div id="photo_view" class="photo_view" onClick="clubs.wall_photo_view_setEvent(event)">'+
...
'</div>';
	...
	wall_video_add_box: function(){
		wall.attach_menu('close', 'wall_attach', 'wall_attach_menu');
		Box.Show('attach_videos', 400, 'Ссылка видеозаписи на webmaster.pp.ua', '<div style="padding:15px;"><input  type="text"  placeholder="Введите ссылку видеозаписи на webmaster.pp.ua.."  class="videos_input" id="video_attach_lnk" style="width:355px;margin-top:10px" /></div>', lang_box_canсel, 'Прикрпепить', 'groups.wall_video_add_select()');
	...
	wall_video_add_select: function(){
		...
		if(video_attach_lnk != 0){
			$('#box_loading').show();
			ge('box_butt_create').disabled = true;
			$.post('/index.php?go=clubs&act=select_video_info', {video_id: data[1]}, function(row){
				if(row == 1){
	...
	wall_add_like: function(rec_id, user_id, type){
		...
		if(type == 'uPages')
			...
			$.post('/index.php?go=clubs&act=wall_like_yes', {rec_id: rec_id});
	},
	wall_remove_like: function(rec_id, user_id, type){
		...
		if(type == 'uPages')
			...
			$.post('/index.php?go=clubs&act=wall_like_remove', {rec_id: rec_id});
	},
	wall_like_users_five: function(rec_id, type){		
		...
		if(!ge('like_cache_block'+rec_id) && $('#wall_like_cnt'+rec_id).text() && $('#update_like'+rec_id).val() == 0){
			if(type == 'uPages'){
				...
				$.post('/index.php?go=clubs&act=wall_like_users_five', {rec_id: rec_id}, function(data){
					$('#likes_users'+rec_id).html(data+'<span id="like_cache_block'+rec_id+'"></span>');
					$('#public_likes_user_block'+rec_id).show();
				});
			}
	...
	wall_all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=clubs&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 625, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
	wall_tell: function(rec_id){
		$('#wall_tell_'+rec_id).hide();
		myhtml.title_close(rec_id);
		$('#wall_ok_tell_'+rec_id).fadeIn(150);
		$.post('/index.php?go=clubs&act=wall_tell', {rec_id: rec_id}, function(data){
			if(data == 1)
				addAllErr(lang_wall_tell_tes);
		});
	},
	wall_tell_c: function(id){
		$('#wall_ok_tell_g').fadeIn(150);
		$.post('/index.php?go=clubs&act=wall_tell_c', {id: id}, function(data){
			if(data == 1)
				addAllErr(lang_wall_tell_tes);
		});
	},
	all_people: function(public_id, page_num){
		...
		Box.Page('/index.php?go=clubs&act=all_people', 'public_id='+public_id+'&num='+num+page, 'all_peoples_users_'+public_id+page_num, 625, 'Участники сообщества', lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
	all_groups_user: function(for_user_id, page_num){
		...
		var num = $('#clubs_num').text();
			
		Box.Page('/index.php?go=clubs&act=all_groups_user', 'for_user_id='+for_user_id+'&num='+num+page, 'all_clubs_users_'+for_user_id+page_num, 525, 'Интересные группы', lang_msg_close, 0, 0, 345, 1, 1, 1, 0, 1);
	},
	miniature: function(j){
		Page.Loading('start');
		$.post('/index.php?go=clubs&act=miniature&pid='+j, function(d){
			Page.Loading('stop');
			if(d == 1) 
			...
			$(window).keydown(function(event){
				if(event.keyCode == 27) epage.miniatureClose();
			});
		});
	},
	...
	miniatureSave: function(j){
		...
		$.post('/index.php?go=clubs&act=miniature_save&cid='+j, {i_left: i_left, i_top: i_top, i_width: i_width, i_height: i_height}, function(d){
	...
	vPhoto: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=clubs&act=vphoto', {uid: uid, photo: photo, type: type}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
				Box.Info('no_video', lang_dd2f_no, lang_photo_info_text, 300);
				$('html, body').css('overflow-y', 'auto');
...
var boards = {
	...
	send: function(board, id){
	
		var text = $('#answer').val();
		
		if(text != 0 && text != 'Комментировать..'){
			butloading('send', '56', 'enabled', 'Отправить');
			$.post('/index.php?go=board&act=send', {text: text, aid: board, id: id}, function(data){
				$('#answers').append(data);
				$('#answer').val('');
				$('#error_boxing').slideUp();
				butloading('send', '56', 'disabled', 'Отправить');
			});
		} else
			setErrorInputMsg('answer');
			
	},
	favourite: function(board){
		
		$.post('/index.php?go=board&act=favourite', {aid: board}, function(data){
			$('#ok_message').append(data);
		});
			
	},
	closed: function(board){
		
		$.post('/index.php?go=board&act=closed', {aid: board}, function(data){
			$('#ok_message').append(data);
		});
	...
	del: function(id){
		...
		$.post('/index.php?go=board&act=delete', {id: id}, function(){
			$('#note_text'+id).html('<div class="info_center">Комментарий удален</div><div class="info_center"><a href="/" onClick="boards.rewrited('+id+'); return false">Восстановить</a></div>');
		});
			
	},
	rewrited: function(id){
		
		$.post('/index.php?go=board&act=rewrite', {id: id}, function(data){
			$('#note_text'+id).html('');
			$('#note_text'+id).append(data);
		});
	...
	changename: function(id){
		var name = $('#title_name_topics').text();
		Box.Show('changenametopic', 490, 'Редактирование темы', '<div style="padding:20px"><div class="videos_text">Новое название темы:</div><input type="text" class="videos_input" id="title_name_change" maxlength="255" style="width: 435px;" value="'+name+'" /></div>', lang_box_canсel, 'Сохранить', 'boards.change_name('+id+')');
	},
	del_topic: function(id){
		Box.Show('deleteoks', 265, 'Редактирование темы', '<div style="padding:20px">Вы уверены, что хотите удалить эту тему?</div>', lang_box_canсel, 'Удалить', 'boards.delete_topic('+id+')');
	},
	change_name: function(id){
	
		var title = $('#title_name_change').val();
		
		if(title!=0) {
			$.post('/index.php?go=board&act=changename', {title: title, aid:id}, function(data){
				Box.Close('changenametopic');
				$('#title_name_topics').text(title);
			});
		} else 
			setErrorInputMsg('title_name_change');
	},
	delete_topic: function(id){
		
		$.post('/index.php?go=board&act=deletetopic', {aid: id}, function(data){
			$('#ok_message').append(data);
			Box.Close('deleteoks');
		});
	...
	newtopic:function(id){
		...
		var title = $('#title_new_topic').val();
		var text = $('#text_new_topic').val();
	
		$.post('/index.php?go=board&act=newtopic', {aid: id, title: title, text: text}, function(data){
			Page.Go('/topic-'+id+'_'+data);
		});
	
	},
	editform: function(id){
		var text = $('#textform'+id).text();
		$('#boardtext'+id).hide();
		$('#red'+id).show();
		$('#new_message_from'+id).text(text);
	},
	cancelform: function(id){
		$('#boardtext'+id).show();
		$('#red'+id).hide();
	},
	okayform: function(id){
	
		var text = $('#new_message_from'+id).val();
		
		if(text!=0) {
			$.post('/index.php?go=board&act=editmessage', {aid: id, text: text}, function(data){
				$('#textform'+id).text(text);
				$('#boardtext'+id).show();
				$('#red'+id).hide();
				$('#favered'+id).text(data);
			});
		} else 
			setErrorInputMsg('new_message_from'+id);
	}
...
var groups = {
	wall_tell_g: function(id){
		$('#no').show("slow").text('Ваши друзья увидят ссылку на эту группу в своих Новостях.');
	...
	created: function(){
		var f = "'";
		Box.Show('create', 410, 'Создание нового сообщества', '<div style="padding:15px"><div class="videos_text" style="margin-bottom: 5px;">Название</div><input type="text" class="videos_input" id="title" maxlength="65" style="width: 363px;" /><div class="videos_text" style="margin-bottom: 5px;">Вид сообщества</div><input id="value_type" type="hidden" value="1"/><div><div class="radiobtn settings_reason on" onclick="radiobtn.select('+f+'value_type'+f+',1);" style="padding-top:10px;"><div></div>Группа</div><div class="groups_create_about">Подходит для дискуссий и обмена мнениями</div></div><br><div><div class="radiobtn settings_reason" onclick="radiobtn.select('+f+'value_type'+f+',2);"><div></div>Публичная страница</div><div class="groups_create_about">Идеально для распространения новостей и информации</div></div></div>', lang_box_canсel, lang_groups_cretate, 'groups.creat_base()','', 0, 0, 0, 0, 'title');
		$('#title').focus();
	},
	creat_base: function() {
		if($('#value_type').val() == 1) clubs.creat();
		else groups.creat();
	},
	login: function(id){
		$('#yes').hide();
	...
	saveinfo: function(id){
		...
		if(pattern.test(adres_page)){
			butloading('pubInfoSave', 55, 'disabled');
			$.post('/index.php?go=groups&act=saveinfo', {id: id, title: title, descr: descr, comments: comments, audio: audio, contact: contact, videos: videos, adres_page: adres_page, gtype: $('#gtype').val(), web: $('#web').val()}, function(d){
				if(d == 'err_adres')
					Box.Info('err', 'Ошибка', 'Такой адрес уже занят', 130, 1500);
				else
					if($('#prev_adres_page').val() == adres_page)
	...
	editform: function(){
		$('#edittab1').slideDown('fast');
		$('#public_editbg_container').animate({scrollLeft: "+410"});
	},
	editformClose: function(){
		$('#public_editbg_container').animate({scrollLeft: "-410"}, 1000);
		setTimeout("$('#edittab1').slideUp('fast')", 200);
		$('#edittab2').hide();
	},
	edittab_admin: function(id){
	...
	addadmin: function(id){
		var new_admin_id = $('#new_admin_id').val().replace('http://webmaster.pp.ua/u', '');
		var check_adm = $('#admin'+new_admin_id).text();
		if(new_admin_id && !check_adm){
...
'</div>';
	...
	wall_all_liked_users: function(rid, page_num, liked_num){
		...
		Box.Page('/index.php?go=groups&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 625, lang_wall_liked_users, lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);
	},
	wall_tell: function(rec_id){
		$('#wall_tell_'+rec_id).hide();
		myhtml.title_close(rec_id);
	...
	all_groups_user: function(for_user_id, page_num){
		...
		Box.Page('/index.php?go=groups&act=all_groups_user', 'for_user_id='+for_user_id+'&num='+num+page, 'all_groups_users_'+for_user_id+page_num, 625, 'Интересные страницы', lang_msg_close, 0, 0, 'auto', 0, 1, 1, 0, 1);

	},
	subscribes_groups: function(j,i){
		viiBox.start();
		if(!i) i = 1;
		if(i == 1) var z = '';
		else var z = '&friends=1';
		$.post('/index.php?go=groups&act=subscribes'+z, {pid: j}, function(data){
			viiBox.win('subscribes', data);
		});
	},
	page_subscribes_groups: function(j,i,f){
		var page_cnt = $('#offset').val();
		if((parseInt($('#subs_count').text()) > parseInt(page_cnt)*40 || $('#fans_rowsmembers').size() < 40) && $('#loading').val() == 0) {
			if(!i) i = 1;
			if(i == 1) var z = '';
			else var z = '&friends=1';
			$('#loading').val(1);
			if(!f) f = 0;
			$.post('/index.php?go=groups&act=subscribes'+z, {page_cnt: page_cnt, pid: j, fave: f}, function(data){
				var d = data.split('<f>');
				if(!f) $('#offset').val(parseInt($('#offset').val())+1);
				$('#fans_rowsmembers').append(d[1]);
				setTimeout($('#loading').val(0),100)
				$('.miniature_pos').animate({'margin-top': d[0]+'px'});
				if(f) $('#tb_prg').css('visibility', 'hidden');
			});
		}
	},
	friends_groups: function(j,h){
		$('#fans_rowsmembers').html('');
		$('#tb_prg').css('visibility', 'visible');
		if(j == 1) {
			$('#tb_title').html('Подписчики сообщества');
			$('#all_button').addClass('summary_tab_sel').removeClass('summary_tab');
			$('#friends_button').addClass('summary_tab').removeClass('summary_tab_sel');
			$('#offset').val('1');
			$('#type').val('1');
			groups.page_subscribes_groups(h,'1',1);
		} else {
			$('#all_button').removeClass('summary_tab_sel').addClass('summary_tab');
			$('#friends_button').removeClass('summary_tab').addClass('summary_tab_sel');
			$('#tb_title').html('Друзья в сообществе');
			$('#offset').val('1');
			$('#type').val('2');
			groups.page_subscribes_groups(h,'2',1);
		}
...
var audio = {
	...
	addBoxComp: function(){
		Box.Close();
		Box.Show('addaudio_comp', 510, lang_audio_add, '<div class="videos_pad"><div class="videos_text">Ограничения<div class="clear"></div><ul class="listing2"><li style="font-weight:normal;color:#000;font-size:11px;margin-top:10px"><span>Аудиофайл не должен превышать 10 Мб и должен быть в формате MP3.</span></li><li style="font-weight:normal;color:#000;font-size:11px;margin-bottom:15px"><span>Аудиофайл не должен нарушать авторские права.</span></li></ul><div class="button_div fl_l" style="margin-left:170px"><button id="upload">Выбрать файл</button></div><div class="clear"></div></div></div>', lang_box_canсel, lang_album_create, 'audio.send()', 0, 0, 1, 1);
		$('#audio_lnk').focus();
		$('#box_but').hide();
...
var im = {
	...
	settTypeMsg: function(){
		Page.Loading('start');
		$.post('/index.php?go=messages&act=settTypeMsg', function(){
			Page.Loading('stop');
			Page.Go('/im');
		});
	},
	open: function(uid){
		oi = (event.target) ? event.target.id: ((event.srcElement) ? event.srcElement.id : null);
		if(oi != 'okim'+uid && oi != 'deia'+uid) Page.Go('/messages');
		if(oi != 'deia'+uid) {
		$('#imViewMsg').html('<img src="'+template_dir+'/images/loading_im.gif" style="margin-left:285px;margin-top:240px" />');
		$.post('/index.php?go=im&act=history', {for_user_id: uid}, function(d){
			$('#imViewMsg').html(d);
			$('.im_tab2 ').removeClass('im_tab_selected');
			$('#okim'+uid).removeClass('im_tab_over');
			$('#okim'+uid).addClass('im_tab_selected');
			$('.im_scroll').append('<div class="im_typograf"></div>').scrollTop(99999);
			
			var aco = $('#for_featch').text().split(' ');
	...
	overdialog: function(uid){
		if(!$('#okim'+uid).hasClass('im_tab_selected')) {
			$('#okim'+uid).addClass('im_tab_over');
		}
	},
	soverdialog: function(uid){
		$('#okim'+uid).removeClass('im_tab_over');
	},
	viewim: function(uid){
		$('#im_dialog'+uid).addClass('dialogs_row_over');
		$('#deia'+uid).show();
	},
	onviewim: function(uid){
		$('#im_dialog'+uid).removeClass('dialogs_row_over');
		$('#deia'+uid).hide();
	},
	...
	closeddialog: function(u){
		$('#box_loading').show();
		$.post('/index.php?go=im&act=closeddialog', {im_user_id: u}, function(d) {
			$('#okim'+u).detach();
			if($('#dialogs_view').is(':empty') != false) Page.Go('/messages');
		}); 
	},
	...
	search: function(){
		...
		$.post('/index.php?go=im&act=searchmessage', {query: query}, function(d){
			$('#body_messages').html(d);
		});
	}
}
...
var Votes = {
	AddInp: function(){
		...
		$('#addAnswerInp').append('<div id="div_inp_answr_'+$('#answerNum').val()+'"><div class="texta">&nbsp;</div><input type="text" id="vote_answer_'+$('#answerNum').val()+'" class="inpst vote_answer" maxlength="80" value="" style="width:330px" /><div class="mgclr"></div></div>');
...
var deactive = {
	changeReason: function(j){
		$('.settings_reason').removeClass('on');
		$(event.target).addClass('on');
		$('#value_reason').val(j);
		var reason = $(event.target).find('.settings_reason_desc').text();
		if(j != 6) $('#settings_reason_text').val(reason);
		if(j == 6) $('#settings_reason_text').focus();
	},
	Go: function(){
		var reason = $('#settings_reason_text').val();
		var type = $('#value_reason').val();
		var nfriends = $('#deact').val();
		if(type <= 0 && type > 6) type = 6;
		if(reason != '' && reason != 'Ваше сообщение..') {
			$.post('/index.php?go=settings&act=deactivego', {reason: reason, type: type, nfriends: nfriends}, function(d){
				if(d == 'already_delete') {
					Box.Info('msg_info', 'Ошибка', 'Ваша страница уже удалена или заблокирована.', 300);
					window.location.reload();
				} else if(d == 'ok') {
					Box.Info('msg_info', 'Успешно', 'Ваша страница удалена, для восстановления обратитесь к Администраторам социальной сети.', 500);
					window.location.reload();
				}
			});
		} else
			setErrorInputMsg('settings_reason_text');
	}
}
...
//AJAXFRIENDS
    ...
    friends:function(id){
        ...
        $('html').css({'overflow':'hidden'});$('body').append('<div id="newbox_miniature"><div class="ajax_strelka"></div><div class="ajaxrequest"><div class="ajaxrequest_pos" style="width: 330px"><div class="box_title" >Заявки в друзья</div><div class="clear"></div><div class="box_conetnt">'+body+'</div><div class="box_footer" ><div class="button_blue fl_r"><button onclick="Profile.miniatureClose()">Закрыть</button></div><div class="fl_l" style="font-size:11px;margin-top:5px;"><a href="/friends/requests" onClick="Profile.miniatureClose(); Page.Go(this.href); return false;">Перейти к списку заявок</a></div></div></div></div></div>');
        });
    },
	msg:function(id){
            $.post('/index.php?go=ajaxmsg',{id:id},function(body){
                    $('body').append('<div id="newbox_miniature"><div class="ajax_strelka"></div><div class="ajaxrequest" style="left:130px;"><div class="ajaxrequest_pos" style="width:455px"><div class="box_title2" >Личные сообщения</div><div class="clear"></div><div class="box_conetnt">'+body+'<br><br></div><div class="box_footer" ><div class="button_div fl_r"><button onclick="Profile.miniatureClose()">Закрыть</button></div><div class="fl_l" style="font-size:11px;margin-top:5px;"><a href="/im" onClick="Profile.miniatureClose(); Page.Go(this.href); return false;">Перейти к списку диалогов</a></div></div></div></div></div>');
            });
    },
	photo:function(id){
            $.post('/index.php?go=ajaxphoto',{id:id},function(body){
                    $('body').append('<div id="newbox_miniature"><div class="ajax_strelka"></div><div class="ajaxrequest" style="left:130px;"><div class="ajaxrequest_pos" style="width:455px"><div class="box_title2" >Отметки на фотографиях</div><div class="clear"></div><div class="box_conetnt">'+body+'<br><br></div><div class="box_footer" ><div class="button_div fl_r"><button onclick="Profile.miniatureClose()">Закрыть</button></div><div class="fl_l" style="font-size:11px;margin-top:5px;"><a href="/albums/newphotos" onClick="Profile.miniatureClose(); Page.Go(this.href); return false;">Перейти к списку новых отметок</a></div></div></div></div></div>');
            });
    }
}
var im_chat = {
...
var epage = {
	saveInfo: function(id){
		...
		if(title != '') {
			if(adres != '') {
				$.post('/index.php?go=editpage&act=saveGeneralInfo', {id:id, title: title, adres: adres, descr: descr, website: website, comments: comments, audio: audio, contact: contact, videos: videos}, function(data){
					$('#info_save').html(lang_infosavepublic);
					$('#info_save').show();
				}); 
			} else setErrorInputMsg('adres');
		} else setErrorInputMsg('title');
	},
	users_page: function(id){
		var page_cnt = $('#page_cnt').val();
		var length = $('#all_users').find('div#gedit_user_members').length;
		if(length < $('#count').val()) {
			if($('#type').val() == 'admin') type = '&tab=admin';
			else type = '';
			$.post('/epage?act=users&pid='+id+type, {page_cnt: page_cnt}, function(data){
				$('#all_users').html(data);
				$('#page_cnt').val(parseInt($('#page_cnt').val())+1);
			});
		}
	...
	editadmin: function(id,type,typetodelete){
		...
		if(in_array(type, atype)) {
			Page.Loading('start');
			$.post('/index.php?go=editpage&act=editadmin&pid='+pid, {id: id, type: type}, function(data){
				Page.Loading('stop');
				if(type == 'deleteadmin') {
					title = 'Удаление руководителя';
					button_ok = 'Разжаловать руководителя';
					function_ok = 'epage.deleteadmin('+id+','+f+typetodelete+f+')';
				} else if(type == 'editadmin') {
					title = 'Редактирование руководителя';
					button_ok = 'Сохранить';
					function_ok = 'epage.saveadmin('+id+','+f+'saveadmin'+f+')';
				} else if(type == 'newadmin') {
					title = 'Назначение руководителя';
					button_ok = 'Назначить руководителем';
					function_ok = 'epage.saveadmin('+id+','+f+'newadmin'+f+')';
				}
	...
	saveadmin: function(id,type){
		...
		$.post('/index.php?go=editpage&act=saveadmin&pid='+pid, {id: id, level: level, blockf: blockf, position: position, email: email, phone: phone, type: type}, function(data){
			Box.Close('editadmin');
	...
	deleteadmin: function(id,type){
		...
		var email = $('#gedit_admbox_email').val();
		var phone = $('#gedit_admbox_phone').val();
		$.post('/index.php?go=editpage&act=deleteadmin&pid='+pid, {id: id, blockf: blockf, position: position, email: email, phone: phone}, function(data){
			Box.Close('editadmin');
			$('#gedit_user_level'+id).detach();
			if(type != 'onfalldelete') Box.Info('msg_info', '<b>Изменения сохранены.</b>', '<span style="font-size:12px;"><b><a href="'+$('#gedit_user_name'+id).attr('href')+'">'+$('#gedit_user_name'+id).html()+'</a></b> убран из списка руководителей.</span>', 380, 2000);
			if(type == 'onfalldelete') epage.delusers(id,pid);
		});
	},
	delusers: function(id,pid) {
		$.post('/index.php?go=editpage&act=deleteusers&pid='+pid, {id: id}, function(data){
			$('#gedit_user_action'+id).hide();
			$('#gedit_user_action'+id).html('Удалить из сообщества');
			$('#gedit_users'+id).hide();
			$('#gedit_user_actions'+id).show();
		});
	},
	rebornusers: function(id) {
		var pid = $('#pid').val();
		textLoad('gedit_user_actions'+id);
		$.post('/index.php?go=editpage&act=rebornusers&pid='+pid, {id: id}, function(data){
			$('#gedit_users'+id).show();
			$('#gedit_user_action'+id).show();
			$('#gedit_user_actions'+id).html('Восстановить');
			$('#gedit_user_actions'+id).hide();
		});
	},
	deleteusers: function(id){ 
		var pid = $('#pid').val();
		textLoad('gedit_user_action'+id);
		if($('#gedit_user_level'+id).html() != '') {
			epage.editadmin(id,'deleteadmin','onfalldelete');
		} else epage.delusers(id,pid);
	},
	delblacklist: function(id,pid){
		textLoad('group_bl_action'+id);
		$.post('/index.php?go=editpage&act=delblacklist&pid='+pid, {id: id}, function(data){
			$('#group_bl_action'+id).html('<span>Вернуть в список</span>');
			$('#group_bl_action'+id).attr('onClick','epage.newblacklist('+id+','+pid+'); return false');
			$('#group_bl_rem'+id).html('удалён из чёрного списка.');
			$('#group_bl_info'+id).hide();
			$('#group_b'+id).addClass('deleted');
			var lenght = parseInt($('#group_bl_rows').find('.group_bl_row').length)-parseInt($('#group_bl_rows').find('.group_bl_row.deleted').length);
			langNumric('gedit_users_summary_members', lenght, 'В сообществе '+lenght+' заблокированный пользователь', 'В сообществе '+lenght+' заблокированных пользователя', 'В сообществе '+lenght+' заблокированных пользователей', 'В сообществе '+lenght+' заблокированный пользователь', 'Нет ни одного заблокированного пользователя');
		});
	},
	newblacklist: function(id,pid){
		textLoad('group_bl_action'+id);
		$.post('/index.php?go=editpage&act=newblacklist&pid='+pid, {id: id}, function(data){
			$('#group_bl_action'+id).html('<span>Удалить из списка</span>');
			$('#group_bl_action'+id).attr('onClick','epage.delblacklist('+id+','+pid+'); return false');
			$('#group_bl_rem'+id).html('');
			$('#group_bl_info'+id).show();
			$('#group_b'+id).removeClass('deleted');
			var lenght = parseInt($('#group_bl_rows').find('.group_bl_row').length)-parseInt($('#group_bl_rows').find('.group_bl_row.deleted').length);
			langNumric('gedit_users_summary_members', lenght, 'В сообществе '+lenght+' заблокированный пользователь', 'В сообществе '+lenght+' заблокированных пользователя', 'В сообществе '+lenght+' заблокированных пользователей', 'В сообществе '+lenght+' заблокированный пользователь', 'Нет ни одного заблокированного пользователя');
		});
	},
	gettingLink: function(){
		var val = $('#link_input').val();
		var pid = $('#pid').val();
		butloading('button_div_link', 140, 'disabled');
		if($('#link_input').val() || $('#link_input') != 'Введите ссылку, которую хотите добавить'){
			$.post('/index.php?go=editpage&act=parse_link&pid='+pid, {lnk: val}, function(d){
				if(d != 'link_error') {
					var date = d.split('<f>');
					img_logo = date[3].replace(/youtube.com/gi, '').split('|');
					if(date[5] == 1) var data = '<input id="imgs_id" value="1" type="hidden"/><div class="box_body" style=""><div class="group_al_thumb select_fix fl_l"><img src="'+date[2]+'" onClick="epage.changePhotos(); return false" id="group_al_thumb_img" class="cursor_pointer"></div><div class="group_al_info fl_l"><input type="text" class="text inpst" id="group_al_title" value="'+date[0]+'" onkeypress="if (event.keyCode == 10 || event.keyCode == 13) epage.newLink()" style="width:258px"><div class="group_al_position">'+date[4]+'</div></div><br class="clear"></div>';
					else var data = '<input id="imgs_id" value="0" type="hidden"/><div class="box_body" style=""><div class="group_al_thumb select_fix fl_l"><img src="'+date[2]+'" onClick="epage.changePhotos(); return false" id="group_al_thumb_img" class="cursor_pointer"></div><div class="group_al_info fl_l"><h2 class="group_al_title">'+date[0]+'</h2><input type="text" class="text inpst" id="group_al_title" value="'+date[1]+'" onkeypress="if (event.keyCode == 10 || event.keyCode == 13) epage.newLink()" style="width:258px"></div><br class="clear"></div>';
					Box.Show('getLink', 410, 'Добавление ссылки', data, lang_box_canсel, 'Добавить', 'epage.newLink()','', 0, 0, 0, 0);
					$('#group_al_title').focus();
					$('.err_red').hide();
					$('.err_fall').hide();
				} else {$('.err_red').show();$('.err_fall').hide();}
				butloading('button_div_link', 140, 'enabled', 'Добавить ссылку');
			});
		}
	},
	newLink: function(){
		var val = $('#link_input').val();
		var pid = $('#pid').val();
		var screen = $('#imgs_id').val();
		var texted = $('#group_al_title').val();
		butloading('box_butt_create', 60, 'disabled');
		$.post('/index.php?go=editpage&act=parse_link&pid='+pid, {lnk: val, screen: screen, actions: 'newLink', texted: texted}, function(d){
			Box.Close('getLink');
			$('.err_red').hide();
			$('.err_fall').show();
			butloading('box_butt_create', 140, 'enabled', 'Добавить');
		});
	},
	editLink: function(id,type){
		butloading('group_l_action_edt'+id, 91, 'disabled');
		var pid = $('#pid').val();
		var photo = $('#group_l_photo'+id).attr('src');
		var title = $('#group_l_title'+id).text();
		var descr = $('#group_l_position'+id).text();
		if(!title) {var tnot = 'Введите название ссылки';title = tnot;}
		if(!descr) {var tnot = 'Введите описание ссылки';descr = tnot;}
		var fth = "if(this.value=='') this.value='"+tnot+"';this.style.color = '#777'";
		var tth = "if(this.value=='"+tnot+"') this.value='';this.style.color = '#000'";
		if(type == 1) var data = '<input id="imgs_id" value="1" type="hidden"/><div class="box_body" style=""><div class="group_al_thumb select_fix fl_l"><img src="'+photo+'" id="group_al_thumb_img" class="cursor_pointer"></div><div class="group_al_info fl_l"><input type="text" class="text inpst" id="group_al_title" value="'+title+'" onkeypress="if (event.keyCode == 10 || event.keyCode == 13) epage.saveLink('+id+')" onblur="'+fth+'" onfocus="'+tth+'" style="width:258px"><div class="group_al_position">'+descr+'</div></div><br class="clear"></div>';
		else var data = '<input id="imgs_id" value="0" type="hidden"/><div class="box_body" style=""><div class="group_al_thumb select_fix fl_l"><img src="'+photo+'" return false" id="group_al_thumb_img" class="cursor_pointer"></div><div class="group_al_info fl_l"><h2 class="group_al_title">'+title+'</h2><input type="text" class="text inpst" id="group_al_title" value="'+descr+'" onkeypress="if (event.keyCode == 10 || event.keyCode == 13) epage.saveLink('+id+')" onblur="'+fth+'" onfocus="'+tth+'" style="width:258px"></div><br class="clear"></div>';
		Box.Show('getLink', 410, 'Редактирование ссылки', data, lang_box_canсel, 'Сохранить', 'epage.saveLink('+id+')','', 0, 0, 0, 0);
		$('#group_al_title').focus();
		butloading('group_l_action_edt'+j, 91, 'enabled', 'Редактировать');
	},
	saveLink: function(j){
		var pid = $('#pid').val();
		var name = $('#group_al_title').val();
		if(name != 'Введите название ссылки' && name != 'Введите описание ссылки') {
			butloading('group_l_action_edt'+j, 91, 'disabled');
			$.post("/index.php?go=editpage&act=saveLink&pid="+pid, {id: j, name: name}, function(d){
				if(d == '2') $('#group_l_title'+j).html(name);
				else $('#group_l_position'+j).html(name);
				Box.Close('getLink');
				butloading('group_l_action_edt'+j, 91, 'enabled', 'Редактировать');
			}); 
		}
	...
	changePhotos: function(){
		var take = parseInt($('#imgs_id').val());
		if(take+1>=img_logo.length) $('#imgs_id').val(0);
		else $('#imgs_id').val(take+1);
		$('#group_al_thumb_img').attr('src', img_logo[$('#imgs_id').val()]);
	},
	drag: function(){
		var pid = $('#pid').val();
		$("#dragndrop ul").sortable({
			cursor: 'move',opacity: 0.9,scroll: false,revert: true,
			update: function(){var order = $(this).sortable("serialize"); $.post("/index.php?go=editpage&act=save_pos_links&pid="+pid, order, function(){});},
			stop: function(event, ui){$(this).find('.group_l_row').removeClass('group_l_row_moving');},
			start: function(event, ui){ui.helper.addClass('group_l_row_moving');}
		});
	},
	delLink: function(j,h){
		butloading('group_l_action_del'+j, 91, 'disabled');
		var pid = $('#pid').val();
		$.post("/index.php?go=editpage&act=delLink&pid="+pid, {id: h}, function(){
			butloading('group_l_action_del'+j, 91, 'enabled', 'Удалить');
			$('#group_l_actions'+j).hide();
			$('#group_l_restore'+j).show();
			$('#group_'+j).addClass('deleted');
			var lenght = parseInt($('.ui-sortable').find('div.group_l_row').length)-parseInt($('.ui-sortable').find('.group_l_row.deleted').length);
			langNumric('gedit_users_summary_members', lenght, 'В сообществе '+lenght+' ссылка', 'В сообществе '+lenght+' ссылок', 'В сообществе '+lenght+' ссылок', 'В сообществе '+lenght+' ссылка', 'На странице еще нет ссылок');
		});
	},
	rebornLink: function(g,j,z){
		butloading('group_l_restore'+z, 91, 'disabled');
		var pid = $('#pid').val();
		var val = $('#group_l_photo'+z).parent().attr('href');
		if(j == 1) texted = $('#group_l_title'+z).text();
		else texted = $('#group_l_position'+z).text();
		$.post('/index.php?go=editpage&act=parse_link&pid='+pid, {lnk: val, screen: g, actions: 'newLink', texted: texted}, function(d){
			$('#group_'+z).removeClass('deleted');
			$('#group_l_actions'+z).show();
			$('#group_l_action_del'+z).attr('onClick','epage.delLink('+z+','+d+'); return false');
			butloading('group_l_restore'+z, 91, 'enabled', 'Восстановить');
			$('#group_l_restore'+z).hide();
			var lenght = parseInt($('.ui-sortable').find('div.group_l_row').length)-parseInt($('.ui-sortable').find('.group_l_row.deleted').length);
			langNumric('gedit_users_summary_members', lenght, 'В сообществе '+lenght+' ссылка', 'В сообществе '+lenght+' ссылок', 'В сообществе '+lenght+' ссылок', 'В сообществе '+lenght+' ссылка', 'На странице еще нет ссылок');
		});
	},
	miniature: function(j){
		Page.Loading('start');
		$.post('/index.php?go=editpage&act=miniature&pid='+j, function(d){
			Page.Loading('stop');
			if(d == 1) 
				addAllErr('Вы пока что не загрузили фотографию.');
			else {
	...
	miniatureSave: function(j){
		...
		var i_height = $('#mi_height').val();
		butloading('miniatureSave', '111', 'disabled', '');
		$.post('/index.php?go=editpage&act=miniature_save&pid='+j, {i_left: i_left, i_top: i_top, i_width: i_width, i_height: i_height}, function(d){
			if(d == 'err') addAllErr('Ошибка');
			else window.location.href = '/'+d;
			butloading('miniatureSave', '111', 'enabled', 'Сохранить изменения');
		});
	...
	vPhoto: function(uid, photo, type){
		Photo.Loading('start');
		$.post('/index.php?go=editpage&act=vphoto', {uid: uid, photo: photo, type: type}, function(d){
			Photo.Loading('stop');
			if(d == 'no_photo'){
				Box.Info('no_video', lang_dd2f_no, lang_photo_info_text, 300);
				$('html, body').css('overflow-y', 'auto');
...
var ads = {
  to:function(){
	$.post('/index.php?go=ads&act=to',function(body){
		$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 540px;padding:15px;"><div class="news_title fl_l" style="color: #2B587A;font-weight: bold;">Таргетированные рекламные объявления</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div class="clear"></div><br>Таргетированная реклама WebMaster.pp.ua — уникальный инструмент, позволяющий всего за несколько минут создать рекламное объявление и начать показывать его Вашим потенциальным клиентам.<br><h1 onClick="reg.step1(); return false" id="reg_lnk">Преимущества таргетированных объявлений</h1><ul class="listing2"><li><span>Вы обращаетесь только к той аудитории, которая Вам необходима.</span></li><li><span>Вы выбираете – платить за уникальные переходы или за показы.</span></li><li><span>Вы платите меньше, создавая эффективные объявления.</span></li><li><span>Вы можете изменить дизайн и текст объявлений в любой момент.</span></li><li><span>Вы получаете подробную статистику эффективности кампаний.</span></li></ul><h1 onClick="reg.step1(); return false" id="reg_lnk">Возможности выбора целевой аудитории безграничны</h1><ul class="listing2"><li><span>Пол, возраст, семейное положение, страна и город проживания.</span></li><li><span>Вуз, факультет, школа, год получения высшего образования.</span></li><li><span>Должности, районы, станции метро и даже улицы.</span></li><li><span>Интересы, увлечения, любимые фильмы, книги, игры и т.д.</span></li></ul><center><div class="ads_intro_box_targeting_image"></div><br><div class="button_blue"><a href="/adscreate"><button style="width:161px">Создать объявление</button></a></div></center><br></div></div></div>');
	});
  },
  zvs:function(){
